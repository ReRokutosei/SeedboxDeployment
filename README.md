...
在我各种搜索之后，（虽然得到的案例不多）知道了我的需求貌似处于一个中间地带，被 DMCA 投诉的对象一般是热门美剧、英剧、好莱坞大片或主流流媒体平台（~~Ne\*flix、Di\*ney+、H\*O~~等）的独家剧集，而某 Mi\*an、某 N\*a 上面这类动漫种子则通常都是经由字幕组重烧录编码，文件哈希已经改变（除了 Raw 生肉搬运、部分由 ~~Ne\*flix~~ 发行的动漫），版权方很难追溯。那么我就想，是不是不一定需要找版权宽松地区的机器？这样选择范围变广了，再考虑到我需要通过 OpenList 在线观看，那在预算有限的情况下，优先选择美西地区的大盘机……
...
- **种子自动下载**：在 qBEE 中配置 RSS 订阅，番剧更新后自动下载到 VPS （限于篇幅，本文不教如何使用 RSS 订阅种子）
- **多端在线观看**：在任意设备的浏览器打开 `https://openlist.yourdomain.com`，直接在线播放已下载的番剧
- **本地备份（可选）**：通过 Air2、SFTP、Rclone 等工具将文件拖回 Windows 本地保存
...
> [!NOTE]
> 为避免纠纷，本文**不提供任何 VPS 商家推荐或购买指导**，请自行选择
>
...
去年发现热门种子容易被国内用户刷下行以平衡上行，可按需在 PBH 中屏蔽所有来自 CN 的 IP：设置 → 规则订阅 → 地区 → 新增 → 输入 `CN`（必须大写）

> [!NOTE]
> PBH 已通过行为检测（BTN 规则、进度判定、吸血识别）自动封禁恶意 Peer，一般无需额外按地理范围封禁。开启后会切断中文社区做种者的连接，可能影响做种效率，酌情使用
...
> [!NOTE]
> 浏览器中 OpenList 暂不支持解析 MKV 容器的内嵌软字幕
> 
> 遇到无字幕的情况，点击页面下方小按钮跳转到外部播放器即可
> 
> PC 端推荐 MPV，Android 端推荐 VLC
...
> [!CAUTION]
> 若路径填写错误，WebUI 会返回「**不支持的文件类型**」且无法再进入页面
> 
> 此时通过 SSH 编辑 `nano ~/.config/qBittorrent/qBittorrent.conf`
> 
> 将 `WebUI\AlternativeUIEnabled=true` 改为 `false` 即可恢复默认界面
> 
> 或者将 `WebUI\RootFolder=` 改为正确的 VueTorrent 路径即可恢复正常
...
倘若顺利，当你执行 `ssh seedbox-mihomo` 时，mihomo 日志显示的连接类型应为 `Tunnel`
